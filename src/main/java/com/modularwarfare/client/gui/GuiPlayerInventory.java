...
public class GuiPlayerInventory extends InventoryEffectRenderer implements IRecipeShownListener {
    ...
    public static void drawEntityOnScreen(final int posX, final int posY, final int scale, final float mouseX, final float mouseY, final EntityLivingBase ent) {
        ...
        ent.renderYawOffset = (float) Math.atan(mouseX / -360.0f) * 360.0f;
        ent.rotationYaw = (float) Math.atan(mouseX / -360.0f) * 360.0f;
        ent.rotationPitch = -(float) Math.atan(mouseY / 40.0f);
